...
func loadPolicyLine(line CasbinRule, model model.Model) error {
	var p = []string{line.Ptype,
		line.RoleID, line.Path, line.Method,
		line.CheckAuth, line.V4, line.V5}
...
func (a *Adapter) RemoveFilteredPolicyCtx(ctx context.Context, sec string, ptype string, fieldIndex int, fieldValues ...string) error {
	...
	if fieldIndex <= 0 && 0 < fieldIndex+len(fieldValues) {
		line.RoleID = fieldValues[0-fieldIndex]
	}
	...
	if fieldIndex <= 3 && 3 < fieldIndex+len(fieldValues) {
		line.CheckAuth = fieldValues[3-fieldIndex]
	}
...
func (a *Adapter) rawDelete(ctx context.Context, db *gorm.DB, line CasbinRule) error {
	...
	if line.RoleID != "" {
		queryStr += " and v0 = ?"
		queryArgs = append(queryArgs, line.RoleID)
	}
	if line.Path != "" {
		queryStr += " and v1 = ?"
		queryArgs = append(queryArgs, line.Path)
	}
	if line.Method != "" {
		queryStr += " and v2 = ?"
		queryArgs = append(queryArgs, line.Method)
	}
	if line.CheckAuth != "" {
		queryStr += " and v3 = ?"
		queryArgs = append(queryArgs, line.CheckAuth)
	}
	if line.V4 != "" {
		queryStr += " and v4 = ?"
...
func (a *Adapter) Preview(rules *[]CasbinRule, model model.Model) error {
	...
	for i, rule := range *rules {
		r := []string{rule.Ptype,
			rule.RoleID, rule.Path, rule.Method,
			rule.CheckAuth, rule.V4, rule.V5}
		index := len(r) - 1
		for r[index] == "" {
			index--
		}
...
func (c *CasbinRule) queryString() (interface{}, []interface{}) {
	...
	if c.RoleID != "" {
		queryStr += " and v0 = ?"
		queryArgs = append(queryArgs, c.RoleID)
	}
	if c.Path != "" {
		queryStr += " and v1 = ?"
		queryArgs = append(queryArgs, c.Path)
	}
	if c.Method != "" {
		queryStr += " and v2 = ?"
		queryArgs = append(queryArgs, c.Method)
	}
	if c.CheckAuth != "" {
		queryStr += " and v3 = ?"
		queryArgs = append(queryArgs, c.CheckAuth)
	}
	if c.V4 != "" {
		queryStr += " and v4 = ?"
...
func (c *CasbinRule) toStringPolicy() []string {
	...
	if c.RoleID != "" {
		policy = append(policy, c.RoleID)
	}
	if c.Path != "" {
		policy = append(policy, c.Path)
	}
	if c.Method != "" {
		policy = append(policy, c.Method)
	}
	if c.CheckAuth != "" {
		policy = append(policy, c.CheckAuth)
	}
	if c.V4 != "" {
