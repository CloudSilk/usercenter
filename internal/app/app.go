...
func CreateAPP(md *APP) error {
	duplication, err := store.Client().CreateWithCheckDuplication(md, "name = ?", md.Name)
	if err != nil {
		return err
	}
	if duplication {
...
func DeleteAPP(id string) (err error) {
	return store.DB().Unscoped().Delete(&APP{}, "id=?", id).Error
}
...
func QueryAPP(req *QueryAPPRequest, resp *QueryAPPResponse) {
	...
	resp.Records, resp.Pages, err = store.Client().PageQuery(db, req.PageSize, req.PageIndex, orderStr, &resp.Data, nil)
	if err != nil {
...
func GetAllAPPs() (mds []*APP, err error) {
	err = store.DB().Preload("Props").Where("enable=1").Find(&mds).Error
	return
}

func GetAPPById(id string) (md APP, err error) {
	err = store.DB().Preload("Props").Where("id = ?", id).First(&md).Error
	return
}
...
func UpdateAPP(md *APP) error {
	return store.DB().Transaction(func(tx *gorm.DB) error {
		...
		duplication, err := store.Client().UpdateWithCheckDuplicationAndOmit(tx, md, true, []string{"created_at"}, "id <> ? and name = ?", md.ID, md.Name)
		if err != nil {
			return err
		}
		if duplication {
...
func ExportAllAPPs(req *apipb.CommonExportRequest, resp *apipb.CommonExportResponse) {
	db := store.DB().Model(&APP{}).Preload("Props").Preload(clause.Associations)
	if req.ProjectID != "" {
