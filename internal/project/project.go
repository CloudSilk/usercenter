...
func GetProjectByID(id string) (*Project, error) {
	m := &Project{}
	err := store.DB().Preload("FormComponents").Preload(clause.Associations).Where("id = ?", id).First(m).Error
	return m, err
}

// ExportAllProjects 注:此处原代码查询 Language 表(疑似历史 bug),迁移保持原行为。
func ExportAllProjects(req *apipb.CommonExportRequest, resp *apipb.CommonExportResponse) {
	db := store.DB().Model(&language.Language{}).Preload("FormComponents").Preload(clause.Associations)
	if req.ProjectID != "" {
...
func UpdateProjectAll(m *Project) error {
	duplication, err := store.Client().UpdateWithCheckDuplicationAndOmit(store.DB(), m, true, []string{"created_at"}, "id != ? and  name=? ", m.ID, m.Name)
	if err != nil {
		return err
	}
	if duplication {
